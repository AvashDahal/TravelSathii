...
const Profile = () => {
  const { guideId } = useParams(); // Get guideId from the URL
  const [profile, setProfile] = useState(null);

  useEffect(() => {
    if (guideId) {
      localStorage.setItem("guideId", guideId); // Save guideId to localStorage
    }
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-7 mt-6">
        ...
        <div className="flex flex-col space-y-4 mt-12 ">
          <Link to={'/book'}>
            <button className="w-full bg-orange-500 text-white py-2 px-4 rounded-md hover:bg-orange-400">Book Now</button>
          </Link>
          <Link to={'/chat'}>
          <button className="w-full bg-orange-500 text-white py-2 px-4 rounded-md hover:bg-orange-400">Contact Now</button>
          </Link>
          <Link to={'/addreview'}>
            <button className="w-full bg-orange-500 text-white py-2 px-4 rounded-md hover:bg-orange-400">Add Review</button>
          </Link>
        </div>
      </div>
    </div>
